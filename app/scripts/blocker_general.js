...
const overlayHTML = `
    <div class="afdcontentblockedoverlay">
        <div class="afdcontentblockedoverlay_top">#{{ PARTEI }}Blocker</div>
        <div class="afdcontentblockedoverlay_bottom">
            100% {{ PARTEI }}-inhaltsfrei<br>
...
function parseHtml(html) {
    ...
    return parsed.getElementsByClassName('afdcontentblockedoverlay')[0];
}
 ...
 * @returns {Element}
 */
function overlayNode(partei = "AfD") {
    var regEx = new RegExp('{{ PARTEI }}', 'g');
    return parseHtml(overlayHTML.replace(regEx, partei));
}
 ...
 * @returns {Element}
 */
function overlaySmallNode(partei = "AfD") {
    var regEx = new RegExp('{{ PARTEI }}', 'g');
    return parseHtml(overlayHTMLSmall.replace(regEx, partei));
}
...
export class Blocker {
     ...
     * @param partei partei string
     */
    constructor(selectorList, xpathExpression, partei = 'AfD') {
    ...
    modifyContent(elements) {
        ...
        addBlocker(nodeConfigurations, this.partei);
    }
 ...
 * @param partei
 */
export function addBlocker(nodeConfigurations, partei) {
    nodeConfigurations.forEach(function(nodeConfiguration) {
        ...
        if (!nodeConfiguration.element.classList.contains('afdcontentblocked')) {
            ...
            if (nodeConfiguration.type === 'big') {
                overlay = overlayNode(partei).cloneNode(true);
            } else {
                overlay = overlaySmallNode(partei).cloneNode(true);
            }
