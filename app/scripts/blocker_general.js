 ...
 */
const overlayHTML = `
    <div class="fdpcontentblockedoverlay">
        <div class="fdpcontentblockedoverlay_top">#FDPBlocker</div>
        <div class="fdpcontentblockedoverlay_bottom">
            100% FDP-inhaltsfrei<br>
...
function parseHtml(html) {
    ...
    const overlay = parsed.getElementsByClassName('fdpcontentblockedoverlay')[0];
    return overlay;
}
...
export class Blocker {
    ...
    constructor(selectorList, xpathExpression) {
        ...
        if (xpathExpression)
        {
            ...
            this.xpathExpression = ".//*[contains(text(),'FDP')]";
        }
    }
            ...
            try {
                ...
                while (node) {
                    ...
                    for(let i = 0; i< this.selectorList.length; i++)
                    {
                        ...
                        if (ancestorTeaser)
                        {
                            if (!ancestorTeaser.classList.contains('fdpcontentblocked'))
                            {
      ...
      let mutationObserver = new MutationObserver(mutations => {
        ...
        for(let i=0; i<mutations.length; ++i) {
            ...
            for(let j=0; j<mutations[i].addedNodes.length; ++j) {
                if (mutations[i].addedNodes[j].classList && !mutations[i].addedNodes[j].classList.contains('fdpcontentblockedoverlay')) {
...
export function addBlocker(nodeConfigurations) {
    nodeConfigurations.forEach(function(nodeConfiguration) {
        ...
        if (!nodeConfiguration.element.classList.contains('fdpcontentblocked')) {
            nodeConfiguration.element.classList.add('fdpcontentblocked');
            let overlay;
